<x-ds-page>
    ...
    <x-ds-modal :show="$viewingRun !== null" title="تفاصيل المسيّر" close-action="closeRun" size="lg">
        @if ($viewingRun)
            <p class="ds-text-muted">{{ $viewingRun->month }} — {{ $viewingRun->status }}</p>
            @if ($viewingRun->notes)
            ...
            <x-ds-table>
                ...
                @foreach ($viewingRun->items as $item)
                    ...
                    @foreach ($item->variables ?? [] as $variable)
                        <tr>
                            <td colspan="6" class="ds-text-muted">
                                {{ ($variable['kind'] ?? '') === 'deduction' ? 'خصم' : 'إضافة' }}:
                                {{ $variable['label'] ?? '' }}
                                ({{ $variable['amount'] ?? 0 }}) — السبب: {{ $variable['reason'] ?? '—' }}
                            </td>
                        </tr>
                    @endforeach
                @endforeach
            ...
            @if ($viewingRun->isEditable())
                @can('hr.salaries.manage')
                    <h4 class="ds-section-title">بند متغير (مكافأة / خصم أداء أو غياب) — السبب إلزامي</h4>
                    <x-ds-form-group label="الصف" :error="$errors->first('variableItemId')">
                        <select class="ds-input" wire:model="variableItemId">
                            <option value="">—</option>
                            @foreach ($viewingRun->items as $item)
                    ...
                    <x-ds-form-group label="البيان" :error="$errors->first('variableLabel')">
                        <input type="text" class="ds-input" wire:model="variableLabel">
                    </x-ds-form-group>
                    <x-ds-form-group label="السبب" :error="$errors->first('variableReason')">
                        <input type="text" class="ds-input" wire:model="variableReason">
                    </x-ds-form-group>
                    <x-ds-form-group label="المبلغ" :error="$errors->first('variableAmount')">
                        <input type="number" step="0.01" class="ds-input ds-ltr-num" wire:model="variableAmount">
                    </x-ds-form-group>
                    <button type="button" class="ds-btn ds-btn-primary" wire:click="addVariable">إضافة البند</button>
                @endcan
            @endif
